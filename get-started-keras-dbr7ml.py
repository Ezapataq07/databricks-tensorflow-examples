...
experiment_log_dir = "/dbfs/emanuel.zapata@datalytics.com/tb"
checkpoint_path = "/dbfs/emanuel.zapata@datalytics.com/keras_checkpoint_weights.keras"
...
# MAGIC %md ### Start TensorBoard
# MAGIC
# MAGIC TensorBoard continues to run until you detach the notebook from the cluster.  
# MAGIC To clear the TensorBoard between runs, use this command: `dbutils.fs.rm(experiment_log_dir.replace("/dbfs",""), recurse=True)`
...
def create_new_model():
  model = Sequential()
  model.add(Dense(int(first_layer), input_dim=8, activation="relu"))
  model.add(Dense(int(second_layer), activation="relu"))
  model.add(Dense(1, activation="linear"))
  return model
...
# MAGIC %md When `autolog()` is active, MLflow does not automatically end a run. You must end the run that was started in Cmd 30 before starting and autologging a new run.
...
# MAGIC %md ## Clean up
# MAGIC Detach this notebook from the cluster to stop TensorBoard.
